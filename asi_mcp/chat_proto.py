"""
Chat Protocol for MCP Client Agent

This module implements a chat protocol for the MCP Client Agent, allowing
users to interact with MCP servers via natural language commands.
"""
...
def set_agent_instance(agent_instance):
    ...
    global mcp_agent_instance
    mcp_agent_instance = agent_instance
    ...
    for item in msg.content:
        if isinstance(item, StartSessionContent):
            ...
            await ctx.send(sender, create_text_chat(
                "Welcome to the MCP Client Agent! You can interact with MCP servers using natural language commands.\n\n"
                "Examples:\n"
            ...
            if connect_match:
                ...
                if token:
                    command = f"!connect {url} --token {token}"
                else:
                    command = f"!connect {url}"
