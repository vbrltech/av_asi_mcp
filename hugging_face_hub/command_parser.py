...
from hub_client import HUGGINGFACE_SERVER_URL
...
class CommandParser:
    ...
    def __init__(self):
        ...
        self.command_patterns = {
            CommandType.CONNECT: r"^!connect(?:\s+(.+))?$",  # Made URL optional
    ...
    def _parse_connect_command(self, message: str) -> ParsedCommand:
        ...
        Format: !connect [--token TOKEN] [--token-env-var VAR_NAME]
        URL is optional and ignored since we use a hardcoded URL.
        ...
        try:
            args_str = match.group(1) if match.groups()[0] else ""
            args = shlex.split(args_str) if args_str else []
        except ValueError:
            return ParsedCommand(CommandType.UNKNOWN, raw_command=message)
        
        # Extract options (ignore URL)
        kwargs = {}
        ...
        while i < len(args):
            if args[i] == "--token" and i + 1 < len(args):
                ...
                # Skip any URL or other positional arguments
                i += 1
        
        return ParsedCommand(
            CommandType.CONNECT,
            args=[],  # No positional args needed since URL is hardcoded
            kwargs=kwargs,
            raw_command=message
        )
    ...
    def get_help_text(self, command: Optional[str] = None) -> str:
        ...
        if command:
            ...
            if command == "connect":
                return (
                    "**connect [--token TOKEN] [--token-env-var VAR_NAME]**\n"
                    f"Connect to the Hugging Face Hub API server at {HUGGINGFACE_SERVER_URL}.\n\n"
                    "Examples:\n"
                    "```\n"
                    "connect\n"
                    "connect --token your-auth-token\n"
                    "connect --token-env-var HF_TOKEN\n"
                    "```"
                )
            elif command == "disconnect":
                return "**disconnect**\nDisconnect from the Hugging Face Hub API server."
            elif command == "list":
                return "**list**\nList available tools on the Hugging Face Hub API server."
            elif command == "call":
                return (
                    ...
                    "Example:\n"
                    "```\n"
                    "call search-models {\"query\": \"stable-diffusion\"}\n"
                    "```"
                )
                ...
                return (
                    ...
                    "Example:\n"
                    "```\n"
                    "shorthand search-models query=\"stable-diffusion\"\n"
                    "```"
                )
            elif command == "status":
            ...
            # General help
            return (
                "**Hugging Face Hub Agent Commands**\n\n"
                "- **connect [--token TOKEN] [--token-env-var VAR_NAME]**\n"
                f"  Connect to the Hugging Face Hub API server at {HUGGINGFACE_SERVER_URL}\n\n"
                "- **disconnect**\n"
                ...
                "- Model Tools: search-models, get-model-info\n"
                "- Dataset Tools: search-datasets, get-dataset-info\n"
                "- Space Tools: search-spaces, get-space-info\n"
                "- Paper Tools: get-paper-info, get-daily-papers\n"
                "- Collection Tools: search-collections, get-collection-info"
            )