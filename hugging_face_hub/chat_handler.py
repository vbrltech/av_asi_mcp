...
class ChatHandler:
    ...
    def _handle_non_command_message(self, message: str) -> str:
        ...
        return (
            "I'm a Hugging Face Hub Agent that connects to the Hugging Face Hub API.\n\n"
    ...
    async def _handle_connect_command(self, command: ParsedCommand) -> str:
        ...
        Returns:
            Response message
        """
        # Extract token information (ignore URL)
        token = command.kwargs.get("token")
        token_env_var = command.kwargs.get("token_env_var")
        
        # Inform the user that we're using the hardcoded URL
        logger.info("Using hardcoded Hugging Face Hub API server URL")
        
        result = await self.session_manager.connect(token=token, token_env_var=token_env_var)
    ...
    async def _validate_and_fill_parameters(self, tool_name: str, args: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            # Get the tool schema
            schema = await self.session_manager.hub_client.get_schema(tool_name)
    ...
    async def _handle_call_command(self, command: ParsedCommand) -> str:
        ...
        try:
            ...
            return self.result_formatter.format_parameter_validation_error(tool_name, str(e),
                await self.session_manager.hub_client.get_schema(tool_name))
        except Exception as e:
